//Edited by Aditya Subramanian and Sabastian Mugazambi 
...
public class Program {
  ...
  private boolean syntaxCheck(Scanner mScanner) throws SyntaxErrorException {
   StackInterface<Character> stack = new Stack<Character>(100);
   
   //Variable that keeps track of the line that we are checking errors for. 
   int linecount = 0;
   
   //Looping through every line and checking every character to see if its syntax related. 
    while(mScanner.hasNext()){
        String x = mScanner.nextLine();
        linecount++;

        //This boolean will be used to check for the existance of multiline comments. 
        boolean multiline = false;
        
        //Looping through each and every character and checking if its one of our syntax characters. 
        for(int i= 0 ; i< x.length(); i++) {
                char check = x.charAt(i);
            
                if (i+1 < x.length()){

                    //Checking for comments and ignoring the rest of the line if comment is found. 
                    if (check == '*' && x.charAt(i+1) == '/'){
                      multiline = false;
                      }
                ...
                //Adding the initial syntax characters to the top of the stcack. 
                if ((check == '[' || check == '{' || check == '(') && multiline == false) {
                    if(check == '['){
                        stack.push(']');
                    }
                    if(check == '{'){
                ...
                //Checking if the current character has a match which is ontop of the stack as expected. 
                if ((check == ']' || check == '}' || check == ')') && multiline == false) {
                     if (!(stack.pop() == check)){
                            throw new SyntaxErrorException("Mismatched " + check + " at line " + linecount);
                     }
                }
    ...
    //Checking if the stack is empty and noting if we are missing a character. 
    if(stack.isEmpty()){
        return true;
    }
    throw new SyntaxErrorException("missing " + stack.pop());
